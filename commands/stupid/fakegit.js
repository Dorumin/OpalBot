...
module.exports = (OpalBot) => {
    ...
    out.peasants = {};
    out.peasants.fg = 'fakegit';
    out.peasants.fagit = 'fakegit';
    out.peasants.fakegit = (message, content, lang) => {
        let repo = message.guild.name.replace(/[\s-]+(\S)/g, (_, char) => char.toUpperCase());
        let branch = message.channel.name;
        let commit = defaults[Math.floor(Math.random() * defaults.length)];
        ...
        if (content) {
            ...
            const splitSquared = split[0].split(':');
            if (splitSquared.length > 1) {
                repo = splitSquared[0];
                branch = splitSquared[1];
                split.splice(0, 1);
            }
        ...
        message.channel.send({
            embed: {
                title: OpalBot.i18n.msg('title', 'fakegit', repo + ':' + branch, lang),
                color: 0x7289da,
                url,
